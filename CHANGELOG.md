...
- 提供大小写敏感的 `ActionParameter` 与封闭递归 `ActionParameterValue`，以强类型表达六类协议参数值并保持递归集合不可变；提供 `ActionDefinition<P>`、封闭结果 `ActionParameterAdapter<P>` 和不可变 `ActionRegistry`，拒绝重复注册与参数 Class 错配。
