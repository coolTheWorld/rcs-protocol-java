...
- 提供 Factsheet 强类型根模型、头部无关的 `FactsheetContent` 能力聚合、平铺确定性 Codec 与完整前三层入站 Validator；Mobile Robot 在活跃连接会话中确定性生成强类型发布 Effect，Fleet Control 以成功/拒绝 Event、最近消息 State 和不泄露扩展键值的能力/诊断 Effect 建立接收边界。
