...
- 提供 Factsheet 强类型根模型、头部无关的 `FactsheetContent` 能力聚合、平铺确定性 Codec 与完整前三层入站 Validator；Mobile Robot State 独立保存 Factsheet Topic 计数器和最近消息，发布 Event 不接受调用方 Header，默认状态机在活跃连接会话中确定性生成强类型发布 Effect。
