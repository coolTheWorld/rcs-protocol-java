...
- 核心代码行覆盖率不低于 90%，分支覆盖率不低于 85%；分支覆盖率以 100% 为持续目标，新增或修改的生产类应达到 100%。
- 确实无法覆盖的分支必须在 Spec 的 `docs/branch-coverage.md` 逐项登记位置、条件、证据、风险、补偿措施和复审点；每个已识别的强制规范或安全分支仍必须有场景测试，不能用例外替代。
